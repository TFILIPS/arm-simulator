...
use headers::{
    ELFHeader, ELFTableEntry, ProgramHeader, SectionHeader, SymbolTableEntry, ValueError,
};
...
impl ELFFile {
    pub fn load(file_path: &str) -> Result<Self, String> {
        let raw_data = fs::read(file_path).or(Err(String::from("Failed to read the ELF file!")))?; // ALEX: ? propagates errors
    ...
    pub fn get_labels(&self) -> Result<Vec<(u32, String)>, String> {
        ...
        for header in headers
            .iter()
            .filter(|h| h.section_type == SECTION_SYMTAB_KEY)
        {
    ...
    pub fn get_text_section_range(&self) -> Result<(u32, u32), String> {
        ...
        let str_header: &SectionHeader = &headers[self.elf_header.section_str_table_idx as usize];
        let str_table: String = self.load_string_table(str_header)?;
    ...
    pub fn check_header_values(&self) -> Result<(), String> {
        match self.elf_header.check_values(
            ...
            Err(ValueError::Version) => Err(String::from("The ELF file has an invalid version!")),
            Err(ValueError::Type) => Err(String::from("The ELF file is not an executable file!")),
    ...
    fn load_string_table(&self, table_header: &SectionHeader) -> Result<String, String> {
